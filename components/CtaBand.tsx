      ...
      <div className="container-awj">
        <Reveal>
          <div className="relative overflow-hidden rounded-[28px] gradient-bg px-8 py-12 sm:px-14 sm:py-16">
            ...
            <span
              aria-hidden
              className="pointer-events-none absolute -left-24 -top-24 h-64 w-64 rounded-full bg-white/20 blur-[70px]"
            />
            <div className="relative flex flex-col items-start justify-between gap-8 md:flex-row md:items-center">
              <div>
                <h2 className="text-2xl font-bold leading-[1.45] text-white sm:text-[32px] sm:leading-[1.35]">
                  {t.titleTop}
                  <br className="hidden sm:block" /> {t.titleBottom}
                </h2>
                <p className="mt-3 max-w-lg text-[16px] leading-[1.85] text-white/85">
                  {t.body}
                </p>
              </div>
              <a
                href="#contact"
                className="group inline-flex shrink-0 items-center gap-2 rounded-full bg-navy-950 px-7 py-3.5 font-semibold text-white transition-transform hover:scale-[1.03] active:scale-95"
              >
                {t.button}
